...
st.caption("Holdings via Financial Modeling Prep · Prices & 52W via Yahoo Finance · Options Sentiment")

# ── Sidebar ───────────────────────────────────────────────────────────────────
with st.sidebar:
    st.header("⚙️ Settings")

    st.markdown("### 🔑 FMP API Key")
    st.markdown(
        "Get your **free** key at [financialmodelingprep.com](https://financialmodelingprep.com/register) "
        "— no credit card needed, 250 req/day free."
    )
    api_key = st.text_input("Paste your FMP API key here", type="password", placeholder="your_api_key_here")
    ...
    quick = st.selectbox(
        "",
        ["(type your own below)", "QQQ","SPY","VOO","VTI","XLK","SMH","XLE","XLF",
         "XLV","ARKG","ICLN","ITA","PAVE","BOTZ","IBB","VNQ","XLY","XLU","XLC"],
    )

    load_options = st.checkbox(
        "Load Options Sentiment",
        value=False,
        help="Fetches options chain for every holding — can take 1–3 min for large ETFs.",
    )
...
with col_in:
    ticker_input = st.text_input("Enter ETF Ticker", value=default_val).upper().strip()
with col_btn:
...
# ── API key check ─────────────────────────────────────────────────────────────
# Allow key from Streamlit secrets too (for deployment convenience)
FMP_KEY = api_key or st.secrets.get("FMP_KEY", "")
...
if not FMP_KEY:
    st.info(
        ...
        "Free tier: **250 requests/day** — more than enough for this dashboard."
    )
    st.stop()
...
def get_holdings_fmp(ticker: str, key: str) -> pd.DataFrame:
    """Fetch ETF holdings from Financial Modeling Prep."""
    ...
    try:
        ...
        keep = [c for c in ["Symbol","Name","Weight %","Shares","Market Value ($)","Updated"] if c in df.columns]
        return df[keep]
...
@st.cache_data(ttl=3600)
def get_etf_profile_fmp(ticker: str, key: str) -> dict:
    """Fetch ETF profile/overview from FMP."""
    url = f"https://financialmodelingprep.com/api/v3/etf-info?symbol={ticker}&apikey={key}"
    try:
        resp = requests.get(url, timeout=15)
        data = resp.json()
    ...
    for sym in symbols:
        try:
            ...
            rows.append({
                ...
                "52W Low": lo52, "52W High": hi52,
                "52W Range %": rng, "% from 52W High": frhi,
            })
        except Exception:
...
if run or ticker_input:
    ...
    # ETF profile from FMP
    with st.spinner(f"Loading {ticker_input} profile…"):
        profile = get_etf_profile_fmp(ticker_input, FMP_KEY)

    etf_name    = profile.get("etfName", ticker_input)
    etf_aum     = profile.get("aum")
    etf_expense = profile.get("expenseRatio")
    etf_nav     = profile.get("nav") or profile.get("price")
    etf_holdings_cnt = profile.get("holdingsCount")
    etf_dom     = profile.get("domicile","–")
    etf_incp    = profile.get("inceptionDate","–")

    st.subheader(f"{ticker_input}  —  {etf_name}")

    m1, m2, m3, m4, m5 = st.columns(5)
    m1.metric("NAV / Price",    f"${float(etf_nav):,.2f}"        if etf_nav     else "N/A")
    ...
    m4.metric("Total Holdings", etf_holdings_cnt                  if etf_holdings_cnt else "N/A")
    m5.metric("Inception",      etf_incp)

    st.divider()

    # Holdings from FMP
    with st.spinner(f"Fetching {ticker_input} holdings…"):
        holdings_df = get_holdings_fmp(ticker_input, FMP_KEY)

    if holdings_df.empty:
        st.error(
            f"❌ No holdings returned for **{ticker_input}**.\n\n"
            "Check your API key is valid, or the ticker may not be supported on FMP's free tier.\n\n"
            f"Verify: https://financialmodelingprep.com/financial-data/etf-holdings?symbol={ticker_input}"
        )
        st.stop()

    symbols = holdings_df["Symbol"].tolist()
    st.success(f"✅  {len(symbols)} holdings loaded for **{ticker_input}**")

    # Prices + 52W
    with st.spinner("Fetching real-time prices and 52-week data…"):
        price_df = get_price_52w(symbols)

    # Options
    if load_options:
        with st.spinner(f"Fetching options for {len(symbols)} stocks… (may take 1–3 min)"):
    ...
    s3.metric("Near 52W High (≤5% off)", int(near_hi))
    ...
    # ── Options sentiment chart ───────────────────────────────────────────────
    if load_options:
        st.divider()
        st.subheader("📊 Options Sentiment — Put/Call Ratio per Holding")
        ...
        if not pc_df.empty:
            ...
            ax_o.axvline(x=0.70, color="green",  linestyle=":", linewidth=1, label="Bullish 0.70")
            ax_o.axvline(x=0.85, color="orange", linestyle=":", linewidth=1, label="Bearish 0.85")
            ax_o.axvline(x=1.00, color="black",  linestyle="--", linewidth=0.8, label="Neutral 1.0")
    ...
    st.divider()
    st.caption(
        "Holdings: Financial Modeling Prep (free tier) · "
        "Prices & 52W: Yahoo Finance (cached 15 min) · "
        "Options: Yahoo Finance (cached 1 hr) · Built with Streamlit"
    )
